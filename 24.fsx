...
let run boost =
        ...
        let (|Cohort|_|) str =
            ...
            | Some v ->        
                ...
                Some {
                    ...
                    Immunities = immu;
                    Weaknesses = weak;
                    Damage = (v.[4] |> int) + (if army = ImmuneSystem then boost else 0);
    ...
    let mutable mc = startingCohorts
    ...
    let mutable us = startingCohorts |> List.sumBy (fun x -> x.Units)
    while ((census mc ImmuneSystem) > 0 && (census mc Infection) > 0) do
        mc <-
            ...
            |> attacking
        let us2 = mc |> List.sumBy (fun c -> c.Units)
        if us = us2 then failwith "Infinite loop"
        us <- us2
    (mc.[0].Army,boost,mc |> List.sumBy (fun c -> c.Units))

run 0 |> dump "Part 1 (26343): %A"
run 57 |> dump "Part 2 (5549): %A"
